...
import { getMyBills } from '@/lib/api';
import { SPALink } from '@/components/SPALink';
import {
    FaFileInvoiceDollar, FaSearch, FaMoneyBillWave,
...
export default function MyBillsPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const res = await getMyBills({ search });
                setBills(res.data || []);
            } catch (err) {
                console.error(err);
            } finally {
    ...
    const pageTitle = user?.role === 'patient' ? 'My Billing' : 'Patient Billing';
    const pageSubtitle = user?.role === 'patient' ? 'View your invoices and payment history' : 'View assigned patient billing (Read-only)';
        ...
        <div className="h-full flex flex-col gap-10 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <StatCard
                    ...
                    variant="indigo"
                    description={`${bills.filter((b) => b.status !== 'paid').length} pending`}
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 p-1 shadow-2xl shadow-gray-200/50">
                <div className="flex flex-col md:flex-row gap-6 items-center p-2">
                    ...
                    <div className="h-14 px-8 bg-white border border-gray-100 rounded-2xl flex items-center gap-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        <FaCheckCircle className="text-emerald-500" /> Read-Only Access
                    </div>
                </div>
            </div>
                    ...
                    <table className="w-full min-w-[1000px] border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50">
                                <th className="py-8 pl-10 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Invoice ID</th>
                                <th className="py-8 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Patient</th>
                                <th className="py-8 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Amount</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading || authLoading ? (
                                ...
                                <tr>
                                    <td colSpan={5}>
                                        <EmptyState title="No Bills Found" subtitle="No billing records available." />
                                    </td>
                                </tr>
                            ) : (
                                bills.map((bill, idx) => (
                                    <tr key={bill.id} className="hover:bg-primary-50/30 group transition-all" style={{ animationDelay: `${idx * 50}ms` }}>
                                        ...
                                        <td className="py-10">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Patient</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-10">
                                            <div className="flex flex-col">
                                                ...
                                                <p className="text-[9px] font-black text-emerald-500 uppercase mt-2 tracking-widest">Total</p>
                                            </div>
                                        </td>
                                        ...
                                        <td className="py-10 pr-10 text-right">
                                            <SPALink href={`billing/${bill.id}`} className="btn-secondary py-3 px-6 text-[10px] font-black uppercase tracking-widest">
                                                View <FaArrowRight className="ml-2 inline-block opacity-50" />
                                            </SPALink>
                                        </td>
                                    </tr>
                                ))
