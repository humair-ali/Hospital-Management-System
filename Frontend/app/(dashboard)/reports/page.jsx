...
export default function ReportsPage() {
  ...
  async function fetchReport() {
    if (!user) return;
    const hasDate = (reportType === 'daily' && reportDate) ||
    ...
    try {
      let res;
      if (reportType === 'daily') res = await getDailyReport(reportDate);
      else if (reportType === 'monthly') res = await getMonthlyReport(reportMonth);
      else res = await getAnnualReport(reportYear);
      ...
      setError(err.message || 'Strategic Analytics Synchronization Failure');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (user && (reportDate || reportType === 'annual')) {
      fetchReport();
    }
  }, [user, reportType, reportDate, reportMonth, reportYear]);

  if (loading || authLoading) return <LoadingState message="Synchronizing Institutional Intelligence..." />;
  if (error) return <div className="p-10 text-center"><div className="bg-red-50 text-red-700 p-8 rounded-3xl border border-red-100 font-bold">{error}</div></div>;
  if (!reportData) return <EmptyState message="No report data available for the selected period." />;
      ...
      {loading || authLoading ? (
        ...
        <div className="space-y-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <StatCard title="Total Revenue" value={`PKR ${(reportData.revenue || 0).toLocaleString()}`} variant="indigo" icon={FaChartLine} />
            <StatCard title="Total Patients" value={reportData.patients_count || 0} variant="dark" icon={FaFileAlt} />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div className="bg-white rounded-[2.5rem] border border-purple-200 p-8 shadow-2xl shadow-gray-200/50 min-h-[30rem] flex flex-col overflow-hidden relative group">
